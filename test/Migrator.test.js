...
contract("Migrator", ([alice, bob, dev, minter]) => {
  ...
  it("should do the migration successfully", async () => {
    ...
    await this.lp1.sync();
    await this.lp1.approve(this.master.address, "100000000000", {
      from: minter,
    });
    await this.master.deposit("0", "2000000", { from: minter });
    assert.equal(
      (await this.lp1.balanceOf(this.master.address)).valueOf(),
      "2000000"
    );
    ...
    assert.equal(
      (await this.lp2.balanceOf(this.master.address)).valueOf(),
      "2000000"
    );
